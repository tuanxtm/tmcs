...
/**
 * Extra feedSection cursor popup labels: empty / item / view-all.
 */
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await addColumnIfMissing(db, 'pages_blocks_feed_section_locales', 'cursor_popup_empty', 'text')
  await addColumnIfMissing(db, 'pages_blocks_feed_section_locales', 'cursor_popup_item', 'text')
  await addColumnIfMissing(db, 'pages_blocks_feed_section_locales', 'cursor_popup_view_all', 'text')

  await addColumnIfMissing(
    db,
    '_pages_v_blocks_feed_section_locales',
    'cursor_popup_empty',
    'text',
  )
  await addColumnIfMissing(db, '_pages_v_blocks_feed_section_locales', 'cursor_popup_item', 'text')
  await addColumnIfMissing(
    db,
    '_pages_v_blocks_feed_section_locales',
    'cursor_popup_view_all',
    'text',
  )
}
