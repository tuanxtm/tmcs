...
/** Parse CSS color with optional alpha (#RRGGBBAA, rgba/hsla, or opaque hex/named). */
const parseColorWithOpacity = (input: string): { color: THREE.Color; opacity: number } => {
  const value = input.trim();
  const hex8 = /^#([0-9a-f]{8})$/i.exec(value);
  if (hex8) {
    const h = hex8[1];
    return {
      color: new THREE.Color(`#${h.slice(0, 6)}`),
      opacity: parseInt(h.slice(6, 8), 16) / 255
    };
  }
  const rgba =
  ...
  if (rgba) {
    ...
    let opacity = 1;
    if (rgba[4] != null) {
      opacity = rgba[4].endsWith('%') ? parseFloat(rgba[4]) / 100 : Number(rgba[4]);
    }
...
const createAsciiAtlas = (chars = ASCII_CHARS) => {
  // Tight monospace-like cells (narrower than tall) so glyphs sit like text
  const cellW = 40;
  const cellH = 64;
  ...
  // Prefer site mono (Geist Mono). Read the resolved family — canvas can't use CSS var().
  const rootStyle = getComputedStyle(document.documentElement);
  const mono =
    rootStyle.getPropertyValue('--font-geist-mono').trim() ||
    'ui-monospace, SFMono-Regular, Menlo, Consolas, monospace';
  // Oversized so ink fills the cell with almost no padding between neighbors
  ctx.font = `bold ${Math.floor(cellH * 1.05)}px ${mono}`;
...
float maskAscii(vec2 p, float visible, float intensity){
  if (visible < 0.001) return 0.0;
  float idx = floor(clamp(intensity, 0.0, 1.0) * (uAsciiCount - 0.001));
  float u0 = idx / uAsciiCount;
  float u1 = (idx + 1.0) / uAsciiCount;
  vec2 uv = vec2(mix(u0, u1, p.x), p.y);
  float g = texture(uAsciiAtlas, uv).r;
  return visible * g;
}
...
void main(){
  ...
  // fontSize is the glyph cell height (device px). Monospace aspect.
  vec2 cellSize = vec2(uFontSize * 0.58, uFontSize);
  vec2 asciiId = floor(fragCoord / cellSize);
  vec2 asciiUV = fract(fragCoord / cellSize);

  // Sample noise per character cell so neighbors can differ,
  // then Bayer-dither for individual character on/off.
  vec2 cellCenter = (asciiId + 0.5) * cellSize;
  vec2 uv = cellCenter / uResolution * vec2(aspectRatio, 1.0);
  ...
  float bayer = Bayer8(asciiId) - 0.5;
  float on = step(0.5, feed + bayer);
  float intensity = clamp(feed + 0.5, 0.0, 1.0);
  float M = maskAscii(asciiUV, on, intensity);
...
const AsciiBlast: React.FC<AsciiBlastProps> = ({
  fontSize = 12,
  color = '#B497CF',
  className,
  style,
  ...
  const threeRef = useRef<{
    ...
    uniforms: {
      ...
      uAsciiAtlas: { value: THREE.Texture };
      uAsciiCount: { value: number };
    };
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const { color: parsedColor, opacity } = parseColorWithOpacity(color);
      const uniforms = {
        ...
        uFontSize: { value: fontSize * renderer.getPixelRatio() },
      ...
      const setSize = () => {
        ...
        uniforms.uFontSize.value = fontSize * renderer.getPixelRatio();
      };
      setSize();
      ...
      threeRef.current = {
        ...
        touch,
        liquidEffect,
        asciiAtlas: asciiAtlas.texture
      };
    } else {
      const t = threeRef.current!;
      const { color: parsedColor, opacity } = parseColorWithOpacity(color);
      t.uniforms.uFontSize.value = fontSize * t.renderer.getPixelRatio();
      t.uniforms.uColor.value.copy(parsedColor);
      t.uniforms.uOpacity.value = opacity;
  ...
  return (
    <div
      ...
      style={style}
      aria-label="AsciiBlast interactive background"
    />
  );
};

export default AsciiBlast;
