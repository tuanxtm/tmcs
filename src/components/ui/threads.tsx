...
export type ThreadsProps = Omit<React.HTMLAttributes<HTMLDivElement>, 'color'> & {
  color?: [number, number, number]
  amplitude?: number
  distance?: number
  // Wave animation speed. Default 0.1 matches the shader's built-in pacing.
  // Higher values animate faster; 0 freezes the wave; negative reverses it.
  speed?: number
...
// Half-width of the line core in pixels (UV-corrected). The line shape is a
// Gaussian truncated to zero at d=1, so each line is razor-thin with no halo
// bleeding into neighboring lines.
...
float lineFn(vec2 st, float width, float perc, float offset, vec2 mouse, float time, float amplitude, float distance) {
    ...
    float time_scaled = time * uSpeed + (mouse.x - 0.5) * 1.0;
    ...
    // Razor-thin line profile: smooth peak at y, hard cut to zero at the
    // edge so no halo bleeds into adjacent lines. Uses smoothstep on |d|
    // for a clean edge with no aliasing.
    // sigma scales with the per-line width so visible lines stay readable
    // even at small u_line_width; the line shape is still hard-truncated
    // to zero beyond abs(d)=1 to avoid halo bleed between neighbors.
    float sigma = max(width * 1.5, 1e-6);
    float d = (st.y - y) / sigma;
    float line = 1.0 - smoothstep(0.0, 1.0, abs(d));

    return clamp(
        line * (1.0 - smoothstep(0.0, 1.0, pow(perc, 0.3))),
        0.0,
        1.0
    );
}
...
const Threads: React.FC<ThreadsProps> = ({
  ...
  useEffect(() => {
    ...
    function update(t: number) {
      ...
      const { color, amplitude, distance, speed, enableMouseInteraction } = propsRef.current
