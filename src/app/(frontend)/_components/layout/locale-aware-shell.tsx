 ...
 *
 * Reading `headers()` here (rather than in the layout) keeps the static shell
 * cache-friendly. The parent layout must wrap this component in `<Suspense>`
 * so the shell ships immediately and these dynamic regions stream in.
 *
 ...
 *
 * The site header is no longer rendered here; it lives inside `PageBlocks` so
 * it participates in the page's `gap-y-(--section-gap-y)` rhythm.
 */
